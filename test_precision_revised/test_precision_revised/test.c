...
void digitLeft(unsigned char num[], int* pL)
{
...
void quickTimesMod(unsigned char num1[], int* pL, unsigned char num2[], int l2, unsigned char numP[], int lP)
{
	for (int i = 0; i<=l2-1; i++)
	{
		LowTimes(num1, *pL, num2[i], num1, pL);
		while (-1 != Compare(num1, numP, *pL, lP))
		{
			Subtraction(num1, numP, pL, lP);
		}
		digitRight(num1, pL);
	}
	while (-1 != Compare(num1, numP, *pL, lP))
...
int main()
{
	...
	quickTimesMod(numR, &lR, numK,lK, num2, l2);
	print_num(numK, lK);
